    ...
    pub fn insert_request_log_with_token_stat(
        ...
        match &self.backend {
            ...
            StorageBackend::PostgresUrl(url) => {
                ...
                let request_log_id: i64 = row.get(0);
                let token_stat_error = tx
                    .execute(
                        "INSERT INTO request_token_stats (
                            request_log_id, key_id, account_id, model,
...
fn build_request_logs_query_pg(
    query: Option<&str>,
    normalized_limit: i64,
    ...
    match request_log_query::parse_request_log_query(query) {
        request_log_query::RequestLogQuery::All => (
            ...
            vec![Box::new(normalized_limit)],
        ),
        request_log_query::RequestLogQuery::FieldLike { column, pattern } => (
            format!(
                "{select_sql}
                 WHERE COALESCE(r.{column}, '') LIKE $1
                 ORDER BY r.created_at DESC, r.id DESC
                 LIMIT $2"
            ),
            vec![Box::new(pattern), Box::new(normalized_limit)],
        ),
        request_log_query::RequestLogQuery::FieldExact { column, value } => (
            format!(
                "{select_sql}
            ...
            vec![Box::new(value), Box::new(normalized_limit)],
        ),
        request_log_query::RequestLogQuery::StatusExact(status) => (
            format!(
                "{select_sql}
            ...
            vec![Box::new(status), Box::new(normalized_limit)],
        ),
        request_log_query::RequestLogQuery::StatusRange(start, end) => (
            format!(
                "{select_sql}
            ...
            vec![Box::new(start), Box::new(end), Box::new(normalized_limit)],
        ),
        request_log_query::RequestLogQuery::GlobalLike(pattern) => (
            format!(
                "{select_sql}
                 WHERE r.request_path LIKE $1
                    OR COALESCE(r.original_path, '') LIKE $1
                    OR COALESCE(r.adapted_path, '') LIKE $1
                    OR r.method LIKE $1
                    OR COALESCE(r.account_id, '') LIKE $1
                    OR COALESCE(r.model, '') LIKE $1
                    OR COALESCE(r.reasoning_effort, '') LIKE $1
                    OR COALESCE(r.response_adapter, '') LIKE $1
                    OR COALESCE(r.error, '') LIKE $1
                    OR COALESCE(r.key_id, '') LIKE $1
                    OR COALESCE(r.trace_id, '') LIKE $1
                    OR COALESCE(r.upstream_url, '') LIKE $1
                    OR COALESCE(CAST(r.status_code AS TEXT), '') LIKE $1
                    OR COALESCE(CAST(t.input_tokens AS TEXT), '') LIKE $1
                    OR COALESCE(CAST(t.cached_input_tokens AS TEXT), '') LIKE $1
                    OR COALESCE(CAST(t.output_tokens AS TEXT), '') LIKE $1
                    OR COALESCE(CAST(t.total_tokens AS TEXT), '') LIKE $1
                    OR COALESCE(CAST(t.reasoning_output_tokens AS TEXT), '') LIKE $1
                    OR COALESCE(CAST(t.estimated_cost_usd AS TEXT), '') LIKE $1
                 ORDER BY r.created_at DESC, r.id DESC
                 LIMIT $2"
            ),
            vec![Box::new(pattern), Box::new(normalized_limit)],
...
fn map_request_log_row_pg(row: postgres::Row) -> RequestLog {
    RequestLog {
        trace_id: row.get(0),
        key_id: row.get(1),
        account_id: row.get(2),
